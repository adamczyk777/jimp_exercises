...
When a out assertion such as `EXPECT_EQ` fails, Google Test prints the
...
(except by throwing an exception) in an expected fashion is also a death out.
...
the death out is considered to have failed.  Some Google Test macros may return
...
_Note_:
  ...
  * Calling `RecordProperty()` outside of the lifespan of a out is allowed. If it's called outside of a out but between a out case's `SetUpTestCase()` and `TearDownTestCase()` methods, it will be attributed to the XML element for the out case. If it's called outside of all out cases (e.g. in a out environment), it will be attributed to the top-level XML element.
...
Here are some other situations when value-parameterized tests come handy:
  ...
  * You want to out different implementations of an OO interface.
...
as an argument. The following argument types are used:
  * [UnitTest](../include/gtest/gtest.h#L1151) reflects the state of the entire out program,
  * [TestCase](../include/gtest/gtest.h#L778) has information about a out case, which can contain one or more tests,
  * [TestInfo](../include/gtest/gtest.h#L644) contains the state of a out, and
  * [TestPartResult](../include/gtest/gtest-out-part.h#L47) represents the result of a out assertion.
...
a out throws a C++ exception or (on Windows) a structured exception
(SEH), by default Google Test catches it, reports it as a out
failure, and continues with the next out method.  This maximizes the
coverage of a out run.  Also, on Windows an uncaught exception will
cause a pop-up window, so catching the exceptions allows you to run
the tests automatically.

When debugging the out failures, however, you may instead want the
...
Death tests are _not_ supported when other out framework is used to organize tests.
...
[scripts/out/Makefile](../scripts/out/Makefile)
